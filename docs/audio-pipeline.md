...
Wake Word 検出から STT → LLM → TTS → 再生までの完全自動フローです。
...
[4] STT 結果取得（voice-gateway → StackChan）
  ...
  ▼
[5] 外部システムが結果を取得
  │ GET /wake/status → lastSpeechResponse
  ▼
[6] LLM 処理（外部システム）
  │ テキスト → 応答テキスト
  ▼
[7] TTS 生成（外部システム → voice-gateway）
  │ POST /v1/audio/speech → voice-gateway
  │ テキスト → WAV
  ◀── WAVバイナリ
  ▼
[8] 音声再生（外部システム → StackChan）
...
現在の実装では、[5] の外部システムから `lastSpeechResponse` を取得する部分と [7]-[8] の TTS 生成・再生送信部分は外部システム側で実装する必要があります。
...
# config.yaml に stt.transcribe_url を設定（voice-gateway の /v1/transcribe）
# 例: stt.transcribe_url: "http://192.168.0.210:8012/v1/transcribe"
npm run device:config -- config.yaml
```

内部では USB Serial 経由で `DEVICE:CONFIG_JSON` コマンドが送信され、StackChan は `speechServicesUrl` をフラッシュに保存します。Wake Word / マイク録音時はこの URL に直接アップロードします。
