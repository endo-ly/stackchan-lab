...
Wake Word 検出から STT → agent runtime → TTS → 再生までの自動フローです。bridge は会話 runtime を持たず、STT callback を agent runtime の1 turnへ渡し、その応答を音声再生します。
...
[5] STT callback（voice-gateway → bridge）
  │ POST /stt/events
  ▼
[6] AgentClient が agent runtime へ turn を送信
  │ agent response text
  ▼
...
AgentClient は STT 済みテキストと会話面識別情報を送り、応答テキストを受け取ります。bridge は agent runtime の LLM provider、memory、tools、内部型を知りません。
